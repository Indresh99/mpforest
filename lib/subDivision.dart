...
class _subDivisionState extends State<subDivision> {
  ...
  List<SubDivisionModel> _notes = List<SubDivisionModel>();
  List<SubDivisionModel> _notesForDisplay = List<SubDivisionModel>();


  List data;
  Future<List<SubDivisionModel>> loadJsonData() async{
    var jsonText = await rootBundle.loadString('assests/SubDivisionJson.json');
    var notes = List<SubDivisionModel>();

    data = json.decode(jsonText);

    for(var not in data){
      notes.add(SubDivisionModel.fromJson(not));
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: Text(_notesForDisplay[index].subDiv_EName, style: TextStyle(
                  fontSize: 22,
              ...
              /*Padding(
                padding: EdgeInsets.all(6),
                child: Text(_notesForDisplay[index].subDiv_HName, style: TextStyle(
                  fontSize: 22,
                  fontWeight: FontWeight.bold,
                ),),
              ),
              ...
              Padding(
                padding: const EdgeInsets.all(4.0),
                child: Text("Phone: "+_notesForDisplay[index].phoneNO),
              ),
              Padding(
                padding: const EdgeInsets.all(4.0),
          ...
          setState(() {
            if(text.isEmpty) {
              ...
              _notesForDisplay = _notes.where((note) {
                var tile = note.subDiv_EName.toLowerCase();
      ...
      return Container(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ListTile(
                title: Text("Sub Division: "+_notesForDisplay[index].subDiv_EName, style: TextStyle(
                    fontFamily: 'Times',
                    fontSize: 22,
                    color: Colors.white
                ),),
              ...
              ListTile(
                title: Text("Sub Division: "+_notesForDisplay[index].subDiv_HName, style: TextStyle(
                    fontFamily: 'Times',
                    fontSize: 22,
                    color: Colors.white
                ),),
...
//                    _num(_notesForDisplay[index].subDiv_HName);
              ...
              ListTile(
                title: InkWell(
                  onTap: (){
                    _ph(_notesForDisplay[index].std.toString()+"-"+_notesForDisplay[index].phoneNO);
                  },
                  child: Text("Phone: "+_notesForDisplay[index].std.toString()+"-"+_notesForDisplay[index].phoneNO, style: TextStyle(
                    fontSize: 20,
                    color: Colors.white,
                    fontFamily: 'Times',
                    fontStyle: FontStyle.normal,
                ...
                leading: IconButton(icon: Icon(Icons.call, color: Colors.white,), onPressed: (){_ph(_notesForDisplay[index].std.toString()+"-"+_notesForDisplay[index].phoneNO);},),
                trailing: IconButton(icon: Icon(Icons.content_copy, color: Colors.white,), onPressed: (){clipboard(_notesForDisplay[index].std.toString()+"-"+_notesForDisplay[index].phoneNO);Navigator.pop(context);}),
